...
export const fetchInactiveUsers = async () => {
  try {
    const response = await axiosWithAuth().get("https://it342-g5-collaboraid.onrender.com/api/auth/inactive-users")
    return response.data
  } catch (error) {
...
// Update the fetchTasks function to use the /task/open endpoint
export const fetchTasks = async () => {
  try {
    // Changed to use the /task/open endpoint as specified
    const response = await axiosWithAuth().get("https://it342-g5-collaboraid.onrender.com/api/task/open")
    return response.data
  } catch (error) {
...
export function useTaskOpen(refreshKey: number) {
  const [tasks, setTasks] = useState([])
...
export function useTaskInProgress(refreshKey: number) {
  const [tasks, setTasks] = useState([])
...
export function useTaskDone(refreshKey: number) {
  const [tasks, setTasks] = useState([])
...
export function useDashboardData() {
  const [stats, setStats] = useState([
    ...
    { title: "Inactive Users", value: "0", change: "0%", changeType: "positive" },
  ])
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch inactive users
        const inactiveUsersResponse = await axiosWithAuth().get("https://it342-g5-collaboraid.onrender.com/api/auth/inactive-users")
        const inactiveUsersCount = inactiveUsersResponse.data.length
        ...
        setStats((prevStats) => [
          ...
          { ...prevStats[3], value: inactiveUsersCount.toString(), change: "0%", changeType: "positive" },
        ])
      } catch (error) {
